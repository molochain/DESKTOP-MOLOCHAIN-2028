...
import { salesOperationsWorkflow } from "./workflows/salesOperationsWorkflow";

// Import tools for MCP server
...
export const mastra = new Mastra({
  ...
  agents: { salesOperationsAgent },
  workflows: { salesOperationsWorkflow },
  mcpServers: {
...
// Register the sales operations workflow to run daily at 9 AM
// Note that we use environment variables first to allow user customization
// Default timezone is America/Los_Angeles and default schedule is daily at 9 AM
registerCronWorkflow(
  `TZ=${process.env.SCHEDULE_CRON_TIMEZONE || 'America/Los_Angeles'} ${process.env.SCHEDULE_CRON_EXPRESSION || '0 9 * * *'}`,
  salesOperationsWorkflow
);
