...
/**
 * GET /api/services/:id/marketplace
 * Get marketplace listing status for a service
 */
router.get('/services/:id/marketplace', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Return marketplace status (mock data for now - can be connected to Mololink)
    const marketplaceStatus = {
      ...
      rating: null,
      listedAt: null,
      marketplaceUrl: `https://mololink.molochain.com/services/${id}`,
...
/**
 * POST /api/services/:id/marketplace/list
 * Request to list a service on the marketplace
 */
router.post('/services/:id/marketplace/list', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Mock successful listing request
    return res.json({
      success: true,
    ...
    console.error('Error submitting marketplace listing:', error);
    return res.status(500).json({
      success: false,
