...
const initializeServer = async () => {
  ...
  // Uses double-submit cookie pattern with cross-subdomain support
  ...
  // Initialize CMS sync service for periodic data synchronization
  setTimeout(async () => {
    try {
  ...
  // Setup environment-specific middleware for client-side routing
  // API routes are already registered above, so Vite won't interfere
  if (app.get("env") === "development") {
    // Dynamic import for Vite in development only (avoid loading dev dependencies in production)
    const { setupVite } = await import('./vite');
    await setupVite(app, server);
  } else {
    // Use static-server in production (no Vite dependencies)
    ...
    if (process.env.NODE_ENV === 'production') {
      setTimeout(async () => {
        try {
          ...
          await enhancedErrorRecoverySystem.handleSystemError(error instanceof Error ? error : new Error(String(error)), 'startup');
        }
      }, 5000);
    }
  });
